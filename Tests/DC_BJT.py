...
for ib in ibsweep:
    ...
    for vce in vcesweep:
        ...
        ic[i] = -x[2,0]
        i = i + 1
    vbe_pyhbsim.append(vbe)
    vce_pyhbsim.append(vcesweep)
    ib_pyhbsim.append(ibsweep)
    ic_pyhbsim.append(ic)
...
for i in range(0, len(ibsweep)):
    plt.plot(vce_pyhbsim[i], ic_pyhbsim[i])
plt.title('PyHBSim BJT Curve Tracer')
plt.xlabel('Vce [V]')
plt.ylabel('Ic [mA]')
plt.grid()
